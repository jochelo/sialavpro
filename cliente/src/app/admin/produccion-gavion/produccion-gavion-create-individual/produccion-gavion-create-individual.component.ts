...
import {irVistaProduccionGavion, storeProduccionGavionIndividual} from '../../../store/actions/produccion-gavion.actions';

declare const contentWayPoint: any;
declare const mobileMenuOutsideClick: any;

@Component({
  selector: 'app-produccion-gavion-create-individual',
  templateUrl: './produccion-gavion-create-individual.component.html',
  styles: []
})
export class ProduccionGavionCreateIndividualComponent implements OnInit {
  ...
  onStore(): any {
    this.store.dispatch(storeProduccionGavionIndividual({
      produccionGavion: this.produccionGavionGroup.value
    }));
  }

  onCancel(): void {
    if (confirm('¿Está seguro de salir del registro de producción individual masiva de gaviones?')) {
